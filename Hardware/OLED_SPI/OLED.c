...
void OLED_Init(void)
{
	...
	OLED_WriteCommand(0xAF);	//开启显示,0xAE为休眠
...
/**
  * 函    数:OLED屏待机
  * 参    数:Data 指定OLED屏功耗模式 0待机 1唤醒
  * 返 回 值:无
  * 说    明:调用此函数后OLED屏幕进入低功耗待机模式或从待机唤醒
  */
void OLED_Power(u8 Data)
{
	...
	{
		OLED_WriteCommand(0xAD);	//设置充电泵
		OLED_WriteCommand(0x8B);    //0x8B开启充电泵	

		OLED_WriteCommand(0xAF);	//0xAF开启显示
	}
	else
	{
		OLED_WriteCommand(0xAE);	//0xAE关闭显示

		OLED_WriteCommand(0xAD);	//设置充电泵
		OLED_WriteCommand(0x8A);    //0x8A关闭充电泵
	}
}
...
/**
  ...
  * 说    明:调用此函数后,要想真正地呈现在屏幕上,还需调用更新函数
  */
void OLED_MyShowChinese(int8_t X, int8_t Y, char* Hanzi, u8 FontSize)	//汉字单字打印;
{
	u8 pIndex;
...
/**
  ...
  * 说    明:调用此函数后,要想真正地呈现在屏幕上,还需调用更新函数
  */
void OLED_ShowString(int8_t X, int8_t Y, char* String, u8 FontSize)	//中英文打印;
{
...
void OLED_ShowImage(int8_t X, int8_t Y0, u8 Width, u8 Height, const u8* Image)
{
	...
	u8  i, j, p = 1;
	/*参数检查,保证指定图像不会超出屏幕范围*/
	if (X + Width <= 0) { return; }        // 如果没有图片在屏幕范围内则打印无意义,故无需打印	
	else if (X > 127) { return; }            // 如果X大于127则打印无意义,故无需打印

	u8 k = 0, l = 0, temp_Height; //(创建Y为负数相关变量)
...
u8 liangdu = 0xFF; 	//亮度
u8 liangdu_UP = 0x00;	//上次亮度位
u8 GUODUspeed = 11;	//执行速率
void OLED_transition(u8 flag)  //屏幕缓灭缓亮,根据其传入的值来决定,缓灭时为阻塞型函数
{
	if (flag)
	{
...
//			OLED_DisplayBuf[Y / 8 + j][X + i] |= Image[(i + pos)%ground_length];// << (Y % 8)
			OLED_DisplayBuf[Y / 8][X + i] |= Image[0 * Width + i + pos] << (Y % 8);  //<<(Y%8显示完整云)
